...
export default function MembersPage() {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    const { data: passes } = await supabase
      ...
      .in('member_id', profiles.map((p) => p.id))

    const passMap = (passes ?? []).reduce<Record<string, MemberPass & { pass_type: { name: string } }>>(
      (acc, p) => { acc[p.member_id] = p as any; return acc },
      {}
    )
  ...
  useEffect(() => {
    ...
    if (showExpiring) {
      list = list.filter((m) => {
        ...
        const days = differenceInDays(parseISO(m.active_pass.expire_date), new Date())
    ...
    <div className="flex flex-col min-h-dvh">
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-2.5 pb-4">
        {isLoading ? (
          ...
          <EmptyState
            title="회원이 없습니다"
            description={search ? '검색 결과가 없습니다' : '+ 버튼으로 회원을 등록하세요'}
          />
        ) : (
          filtered.map((member) => (
...
function MemberListItem({ member }: { member: MemberWithPass }) {
  ...
  const daysLeft = pass ? differenceInDays(parseISO(pass.expire_date), new Date()) : null
  const isExpiringSoon = daysLeft !== null && daysLeft <= 14
  const isLowCount     = pass?.remaining_count !== null && (pass?.remaining_count ?? 0) <= 3
      ...
      <div className="card-hover p-4 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-body-md font-semibold text-on-surface">{member.name}</span>
            {(isExpiringSoon || isLowCount) && (
              <AlertCircle size={14} className="text-error shrink-0" />
            )}
          </div>
          <p className="text-body-sm text-on-surface-variant">{formatPhone(member.phone)}</p>
        </div>
        ...
        <div className="text-right shrink-0">
          {pass ? (
            <>
              ...
              <p className={cn(
                'text-body-sm',
                isExpiringSoon || isLowCount ? 'text-error font-semibold' : 'text-on-surface-variant'
              )}>
