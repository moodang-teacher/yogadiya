...
export default function NewSessionPage() {
  ...
  const [resultModal, setResultModal]   = useState<{ open: boolean; success: boolean }>({
  ...
  const toggleDay = (day: number) => {
    ...
    if (curr.includes(day)) setValue('recurrence_days', curr.filter((d) => d !== day))
    ...
    <div className="min-h-dvh flex flex-col">
      ...
      {step === 'form' && (
        ...
        >
          ...
          {/* 레벨 라벨 */}
          <div className="space-y-1.5">
            <label className="text-label-lg text-on-surface">레벨 뱃지 (선택)</label>
            <input
          ...
          <div className="space-y-1.5">
            ...
            {errors.instructor_id && <p className="text-body-sm text-error">{errors.instructor_id.message}</p>}
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-label-lg text-on-surface">주간 반복 등록</label>
              <Controller
                name="is_recurring"
                control={control}
                render={({ field }) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <input type="time" {...register('start_time')} className="input-field" />
              {errors.start_time && <p className="text-body-sm text-error">{errors.start_time.message}</p>}
            </div>
            <div className="space-y-1.5">
              <label className="text-label-lg text-on-surface">소요 시간 (분)</label>
              <Controller
                name="duration_min"
                control={control}
                render={({ field }) => (
          ...
          <div className="grid grid-cols-2 gap-3">
                ...
                render={({ field }) => (
                  <div className="flex items-center input-field py-2.5 px-2 gap-2">
                    <button type="button" onClick={() => field.onChange(Math.max(1, field.value - 1))}
                      className="w-8 h-8 rounded-full bg-surface-container flex items-center justify-center text-lg font-bold">−</button>
                    <span className="flex-1 text-center font-bold text-body-md">{field.value} 명</span>
                    <button type="button" onClick={() => field.onChange(Math.min(50, field.value + 1))}
                      className="w-8 h-8 rounded-full bg-surface-container flex items-center justify-center text-lg font-bold">+</button>
                  </div>
                )}
              />
            ...
            <div className="space-y-1.5">
              <label className="text-label-lg text-on-surface">대기 인원 설정</label>
              <Controller
                name="waitlist_max"
                control={control}
                render={({ field }) => (
                  <div className="flex items-center input-field py-2.5 px-2 gap-2">
                    <button type="button" onClick={() => field.onChange(Math.max(0, field.value - 1))}
                      className="w-8 h-8 rounded-full bg-surface-container flex items-center justify-center text-lg font-bold">−</button>
                    <span className="flex-1 text-center font-bold text-body-md">{field.value} 명</span>
          ...
          {/* 메모 */}
          <div className="space-y-1.5">
            <label className="text-label-lg text-on-surface">메모 (선택사항)</label>
            <textarea
              {...register('notes')}
              rows={3}
                ...
                return (
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      {!item.excluded && <span className="text-on-primary text-xs">✓</span>}
                    </div>
                    ...
                    <div className="flex-1">
                      <span className={cn(
                        ...
                        dow === 0 ? 'text-error' : dow === 6 ? 'text-blue-500' : 'text-on-surface'
                      )}>
      ...
      <ResultModal
        ...
        onConfirm={() => {
          ...
          if (resultModal.success) router.push('/schedule')
        }}
      />
    </div>
  )
