...
import { format, addDays, addWeeks, subWeeks, addMonths, subMonths, isSameDay, isSameMonth } from 'date-fns'
import { ko } from 'date-fns/locale'
import { createClient } from '@/lib/supabase/client'
import { useAuth } from '@/lib/hooks/useAuth'
import { getWeekDays, getCalendarDays, formatDateShort, isKoreanHoliday, cn } from '@/lib/utils'
...
export default function SchedulePage() {
  ...
  const calDays  = getCalendarDays(baseDate.getFullYear(), baseDate.getMonth() + 1)

  const selectedDateSessions = sessions.filter(
    (s) => s.session_date === format(selectedDate, 'yyyy-MM-dd')
  )

  const sessionCountByDate = sessions.reduce<Record<string, number>>((acc, s) => {
      ...
      <TopHeader
        ...
        rightSlot={
          <div className="flex items-center gap-1">
            ...
            >
              {viewMode === 'week' ? <CalendarDays size={14} /> : <List size={14} />}
              {viewMode === 'week' ? '월간' : '주간'}
            </button>
          </div>
        }
        ...
        <div className="px-4 pt-2 pb-3">
          ...
          <div className="flex items-center justify-between mt-0.5">
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={() => {
                  const prev = viewMode === 'week' ? subWeeks(baseDate, 1) : subMonths(baseDate, 1)
                  setBaseDate(prev)
                  setSelected(prev)
                }}
                className="btn-icon"
              ...
              <button
                onClick={() => { setBaseDate(new Date()); setSelected(new Date()) }}
              ...
              <button
                onClick={() => {
                  const next = viewMode === 'week' ? addWeeks(baseDate, 1) : addMonths(baseDate, 1)
                  setBaseDate(next)
                  setSelected(next)
                }}
                className="btn-icon"
          ...
          <>
                ...
                {weekDays.map((day) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* 수업 있는 날 도트 */}
                      {sessionCountByDate[format(day, 'yyyy-MM-dd')] && !isSelected && (
            ...
            <div className="px-4 space-y-3">
              ...
              {isLoading ? (
                ...
                <EmptyState
                  title="등록된 수업이 없습니다"
                  description={isKoreanHoliday(selectedDate) ? '공휴일입니다' : '+ 버튼으로 수업을 추가하세요'}
                />
              ) : (
              ...
              <div className="grid grid-cols-7 border-b border-outline-variant">
                {['일','월','화','수','목','금','토'].map((d, i) => (
                  <div key={d} className={cn(
                    ...
                    i === 0 ? 'text-error' : i === 6 ? 'text-blue-500' : 'text-on-surface-variant'
                  )}>
                    {d}
                  </div>
                ))}
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        Math.floor(idx / 7) !== 0 && 'border-t border-outline-variant/50',
                        !inMonth && 'opacity-30',
                      )}
                    >
                      <div className={cn(
                        ...
                        !isSelected && isToday && 'border-2 border-secondary-DEFAULT',
                      )}>
                        <span className={cn(
                        ...
                        <div className="flex gap-0.5 mt-0.5">
                          {Array.from({ length: Math.min(count, 3) }).map((_, i) => (
                            <div key={i} className={cn(
                              ...
                              isSelected ? 'bg-on-primary/60' : 'bg-secondary-DEFAULT'
                            )} />
                          ))}
                        </div>
                      )}
