...
export function ResultModal({
  open, type, title, description, confirmLabel = '확인', onConfirm
}: ResultModalProps) {
  const icons = {
    success: <CheckCircle className="text-secondary-container" size={48} strokeWidth={1.5} />,
    error:   <AlertCircle className="text-error" size={48} strokeWidth={1.5} />,
    info:    <Info className="text-primary-container" size={48} strokeWidth={1.5} />,
  }
  const bgColors = {
  ...
  return (
    <Modal open={open} onClose={onConfirm}>
      <div className="p-8 flex flex-col items-center gap-4">
        <div className={cn('w-16 h-16 rounded-full flex items-center justify-center', bgColors[type])}>
          {icons[type]}
        </div>
...
export function Spinner({ size = 24, className }: { size?: number; className?: string }) {
  return (
    <div
      className={cn('animate-spin rounded-full border-2 border-outline-variant border-t-primary', className)}
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
      ...
      {description && (
        <p className="text-body-sm text-on-surface-variant/70 max-w-xs">{description}</p>
      )}
    </div>
  )
}
