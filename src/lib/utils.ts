...
// ── 정원 현황 뱃지 텍스트 ─────────────────────────────────
export function getCapacityBadge(
	current: number,
	max: number,
): {
	text: string;
	...
	if (current >= max)
		return { text: `${current}/${max} 마감`, variant: 'full' };
	if (current >= max * 0.8)
		return { text: `${current}/${max} 예약`, variant: 'warning' };
...
export function canCancelBooking(
	...
	const cutoff = new Date(
		sessionStart.getTime() - cutoffHours * 60 * 60 * 1000,
	);
